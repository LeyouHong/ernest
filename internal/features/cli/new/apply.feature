...
Feature: Ernest apply
  ...
  Scenario: User with project role applies a manifest
    ...
    When I run ernest with "apply definitions/test.yml"
  ...
  Scenario: User with project role applies a manifest with unknown environment name
    Given I'm logged in as "john" / "secret"
    And the user "john" has "<role>" role on project "myapp"
    And the environment "unknown" does not exist in project "myapp"
    When I run ernest with "apply definitions/unknown-environment.yml"
    Then the output should contain "<output>"
  ...
  Scenario: User applies a manifest with missing project name
    Given I'm logged in as "john" / "secret"
    When I run ernest with "apply definitions/missing-project.yml"
  ...
  Scenario: User applies a manifest with missing environment name
    Given I'm logged in as "john" / "secret"
    When I run ernest with "apply definitions/missing-environment.yml"
  ...
  Scenario: User applies a manifest with unknown project name
    Given I'm logged in as "john" / "secret"
    When I run ernest with "apply definitions/unknown-project.yml"
  ...
  Scenario: User applies an incorrectly formatted manifest
    Given I'm logged in as "john" / "secret"
    When I run ernest with "apply definitions/bad.yml"
